...
export default async function handler(req: any, res: any) {
  // Set CORS headers if needed for Vercel
  ...
  if (!apiKey || apiKey === "MY_GEMINI_API_KEY" || apiKey.trim() === "") {
    console.log("[Vercel OS] No active Gemini API key found, running master heuristic simulator...");
    return res.status(200).json(getHeuristicAnalysis(prompt));
  }
